...
def predict_classes(image_name, yolo_model, ACCESS_ID, ACCESS_KEY, print_image = False, from_local_file=False):
    ...
    if from_local_file:
        img = tf.image.decode_image(open(image_name, 'rb').read(), channels=3)
    ...
    for i in range(np.array((scores!=0)[0]).sum()):
        labels.add(class_names[int(classes[0][i])])
